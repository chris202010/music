/**
 * Cloudflare Worker - 音乐网 (尊享音质+全自动滚动歌词版) - 修复接口稳定版
 */
...
<body>
    ...
    <script>
        ...
        // 多节点备用音源解析（解决第三方接口失效问题）
        async function getPlayUrl(songmid, quality) {
            const providers = [
                'https://api.i-meting.com/meting/api?server=tencent&type=url&id=' + songmid,
                'https://api.injahow.cn/meting/?type=url&id=' + songmid
            ];
        ...
        async function getLyricText(songmid) {
            const lyricProviders = [
                'https://api.i-meting.com/meting/api?server=tencent&type=lyric&id=' + songmid,
                'https://api.injahow.cn/meting/?type=lyric&id=' + songmid
            ];

            for (const url of lyricProviders) {
                try {
                    const res = await axios.get(PROXY + url, { timeout: 8000 });
                    let lrc = "";
                    if (typeof res.data === 'string') {
                        lrc = res.data;
                    } else if (res.data?.lyric) {
                        lrc = res.data.lyric;
                    } else if (res.data?.lrc) {
                        lrc = res.data.lrc;
                    }
